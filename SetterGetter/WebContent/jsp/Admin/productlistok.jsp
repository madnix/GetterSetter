...
<body>
	...
	<%-- </c:forEach> --%>
		...
		<tr>
			<td width="10%" align="center">판매자</td>
			<td width="10%" align="center">구매자</td>
			<td width="10%" align="center">구매수량</td>
			<td width="10%" align="center">구매가격</td>
			<td width="10%" align="center">결제금액</td>
			<td width="20%" align="center">입금자</td>
			<td width="20%" align="center">거래은행</td>
			<td width="10%"></td>
		</tr>
		...
		<tr>
			...
			<c:if test="${ count > 0 }">
				...
				<c:if test="${numPageGroup > 1}">
					<a
						href="./productlistok.do?pro_id=${pro_id}&pageNum=${(numPageGroup-2)*pageGroupSize+1 }">[이전]</a>
				</c:if>

				<c:forEach var="i" begin="${startPage}" end="${endPage}">
					<a href="productlistok.do?pro_id=${pro_id}&pageNum=${i}"> [ <c:if
							test="${currentPage == i}"></c:if> ${i} ]
					</a>
				</c:forEach>

				<c:if test="${numPageGroup < pageGroupCount}">
					<a href="productlistok.do?pro_id=${pro_id}&pageNum=${numPageGroup*pageGroupSize+1}">[다음]</a>
				</c:if>
			</c:if>
			</td>
		</tr>
