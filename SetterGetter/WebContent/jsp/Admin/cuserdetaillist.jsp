	...
	<table align="center" width="70%">
		...
		<c:forEach items="${allproduct}" var="proout">
			<tr>
				...
				<td align="center">${proout.recevier_cartdate}</td>
				<td align="center"><a
		...
		<tr>
			<td colspan="7" align="center"><c:if test="${ count > 0 }">
					...
					<c:if test="${numPageGroup > 1}">
						<a
							href="./cuserdetaillist.do?user_id=${usermassage.id}&pageNum=${(numPageGroup-2)*pageGroupSize+1 }">[이전]</a>
					</c:if>

					<c:forEach var="i" begin="${startPage}" end="${endPage}">
						<a href="cuserdetaillist.do?user_id=${usermassage.id}&pageNum=${i}"> [ <c:if
					...
					<c:if test="${numPageGroup < pageGroupCount}">
						<a
							href="cuserdetaillist.do?user_id=${usermassage.id}&pageNum=${numPageGroup*pageGroupSize+1}">[다음]</a>
					</c:if>
				</c:if></td>
		</tr>
	</table>
	...
	<table align="center" width="80%">
		<tr>
			<td colspan="4" align="right"><a
				href="deleteuser.do?user_id=${usermassage.id }"> 삭제gkrl </a></td>
		</tr>
	</table>
</body>
</html>